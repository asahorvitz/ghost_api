...
parser.add_argument('--sampling-topk', type=int, default=5,
                    help='the topk of the distribution to sample from')
    ...
    with open(args.outf, 'w') as outf: #, open('gold_4sent.txt', 'w') as gf:
        if args.task == 'cond_generate':
            ...
            while nsent < args.sents:
                ...
                for i in range(args.words):
                    if i < cond_length:
                        ...
                        samples = torch.multinomial(word_weights, args.sampling_topk)
            ...
            for i in range(args.words):
                ...
                output = model.decoder(output)
                word_weights = output.squeeze().data.div(args.temperature).exp()#.cpu()
                word_idx = torch.multinomial(word_weights, args.sampling_topk)[0]
                #if word_idx == eos_id:
                #    start_word = ending_dict[numpy.random.randint(0,3)]
