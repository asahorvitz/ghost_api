...
import spacy
import numpy as np
from utils import batchify, get_batch, repackage_hidden, load_pickle, load_model, read_w2v, cosine_sim
...
def preprocess(word_string, special_chars, tokenizer):
    ...
    return [word.text.lower() if word.text not in special_chars else word.text for word in tok_text]
...
def replace_oovs(word_list, vocab, oov_handling, vocab_vectors=None, all_vectors=None ):
    ...
    """
    if not check_for_oov(word_list, vocab.word2idx):
        return word_list
    #TODO move oov handling into a dict of string to function so cleaner. But might need globals...
    for i in range(len(word_list)):
        if word_list[i] in vocab.word2idx:
            continue
        if oov_handling == "wordnet":
            new_word = find_wordnet_substitute(word_list[i], vocab.word2idx)
...
if __name__ == '__main__':
    ...
    if args.oov == "wordvec":
        ...
        # still have to init spacy for tokenization but can use a smaller model
        nlp = spacy.load("en_core_web_lg")

    # add special cases to tokenizer TODO move this into a utils function since it is used here and in general_preprocessing
    ...
    if args.apply_disc:
        ...
        decoder = BeamSearchDecoder(st_model, args.beam_size, st_eos_id, verbosity=False,
                                    sep=st_dict.word2idx[args.story_sep], dict=st_dict, temperature=args.story_temp)
